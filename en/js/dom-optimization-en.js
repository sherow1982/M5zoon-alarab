// DOM Optimization System for English Version
// Reduces DOM complexity and improves loading performance
// Mirrors Arabic version functionality

class DOMOptimizerEN {
    constructor() {
        this.observerConfig = {
            childList: true,
            subtree: true,
    ...
    initialize() {
        console.log('Initializing English DOM optimization system...');
    ...
    startOptimization() {
        ...
        console.log('✅ English DOM optimization completed');
    }
    
    optimizeExistingDOM() {
        console.log('Optimizing existing DOM elements for English version...');
        ...
        console.log('📦 English DOM structure optimized');
    }
    ...
    setupLazyLoading() {
        console.log('Setting up advanced lazy loading for English version...');
    ...
    optimizeImages() {
        console.log('Optimizing images for better performance (English)...');
        
        const images = document.querySelectorAll('img');
        let optimizedCount = 0;
        
        images.forEach(img => {
            // Add missing alt attributes with English descriptions
            if (!img.hasAttribute('alt')) {
                img.setAttribute('alt', 'Emirates Gifts Premium Product Image');
                optimizedCount++;
            }
            ...
            // Add error handling with English messages
            if (!img.dataset.errorHandled) {
                img.addEventListener('error', function() {
                    this.style.opacity = '0.7';
                    this.style.filter = 'grayscale(0.5)';
                    console.log('English - Image failed to load:', this.src);
                }, { once: true });
                img.dataset.errorHandled = 'true';
            }
        });
        
        console.log(`🖼️ Optimized ${images.length} English images (${optimizedCount} missing alt attributes fixed)`);
    }
    
    optimizeStyleCalculations() {
        console.log('Optimizing English style calculations...');
    ...
    batchDOMOperations() {
        ...
        // Export batching functions globally for English version
        window.batchReadEN = (fn) => {
            pendingReads.push(fn);
            scheduleFrame();
        };
        
        window.batchWriteEN = (fn) => {
            pendingWrites.push(fn);
            scheduleFrame();
        };
        
        console.log('📊 English DOM batching system activated');
    }
    ...
    optimizeResizeHandling() {
        ...
        window.addEventListener('resize', this.debounce(() => {
            ...
            if (window.batchReadEN) {
                ...
                window.batchWriteEN(() => {
                    ...
                    window.dispatchEvent(new CustomEvent('optimizedResizeEN', {
                        detail: {
        ...
        console.log('📐 English optimized resize handling');
    }
    ...
    setupPerformanceMonitoring() {
        ...
        if ('PerformanceObserver' in window) {
            const observer = new PerformanceObserver((list) => {
                ...
                entries.forEach(entry => {
                    if (entry.entryType === 'measure') {
                        console.log(`⚡ English Performance: ${entry.name} took ${entry.duration.toFixed(2)}ms`);
                    }
                });
            });
            ...
            performance.mark('dom-optimization-en-start');
            
            setTimeout(() => {
                performance.mark('dom-optimization-en-end');
                performance.measure('dom-optimization-en', 'dom-optimization-en-start', 'dom-optimization-en-end');
            }, 2000);
        }
        
        // Monitor First Contentful Paint for English
        this.monitorCoreWebVitals();
    }
    
    monitorCoreWebVitals() {
        // Monitor Core Web Vitals for performance insights
        console.log('📊 English Core Web Vitals monitoring active');
        
        // Simple FCP monitoring for English version
        if ('PerformanceObserver' in window) {
            const fcpObserver = new PerformanceObserver((list) => {
                const entries = list.getEntries();
                entries.forEach(entry => {
                    if (entry.name === 'first-contentful-paint') {
                        console.log(`🎯 English First Contentful Paint: ${entry.startTime.toFixed(2)}ms`);
    ...
    setupLazyLoading() {
        // Advanced lazy loading for content sections in English
        ...
        if ('IntersectionObserver' in window && lazyElements.length > 0) {
            ...
            console.log(`⏳ Set up lazy loading for ${lazyElements.length} English elements`);
        }
    }
    ...
    loadLazyContent(element) {
        ...
        switch (contentType) {
            case 'products':
                ...
                if (element.id && window[element.id + 'LoaderEN']) {
                    window[element.id + 'LoaderEN']();
                }
                break;
                
            case 'reviews':
                // Load reviews using English system
                if (window.loadReviewsForSectionEN) {
                    window.loadReviewsForSectionEN(element);
                }
                break;
    ...
    // Optimize specific operations for English version
    optimizeProductLoading() {
        ...
        productGrids.forEach(grid => {
            ...
            if (products.length > 12) {
                ...
                // Add "Load More" functionality in English
    ...
    toggleOptimization(enabled) {
        if (enabled && !this.isOptimized) {
            ...
            this.isOptimized = false;
            console.log('❌ English DOM optimization disabled');
        }
    }
}

// Create global instance for English version
const domOptimizerEN = new DOMOptimizerEN();

// Export for external access
window.domOptimizerEN = domOptimizerEN;
window.domOptimizer = domOptimizerEN; // Keep compatibility

// Monitor page load time for English version
window.addEventListener('load', () => {
    setTimeout(() => {
        const report = domOptimizerEN.getPerformanceReport();
        if (report) {
            console.log('📊 English Performance Report:', report);
        }
        
        // Final optimization pass
        domOptimizerEN.optimizeProductLoading();
        
        console.log('🚀 English page fully optimized for speed');
    }, 1000);
});

console.log('⚡ English DOM Optimization System Loaded');
